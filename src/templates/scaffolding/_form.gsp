...
<%  excludedProps = Event.allEvents.toList() << 'version' << 'dateCreated' << 'lastUpdated' << 'createdByUserId' << 'lastUpdatedByUserId' << 'status' << 'createdBySessionId' << 'lastUpdatedBySessionId'
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 fieldcontain \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'error')} ${required ? 'required' : ''}">
